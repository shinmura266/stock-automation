...
// UpdateStatementsByCode 指定された銘柄コードの財務情報を取得し、DBに保存
func (s *StatementsService) UpdateStatementsByCode(code string, debug bool) error {
	if code == "" {
	...
	statements, err := s.client.StatementsClient.GetAllStatements(idToken, code, "", debug)
	if err != nil {
	...
	if len(statements.Statements) > 0 {
		if count, err := s.repository.SaveFinancialStatements(statements); err != nil {
			...
			slog.Info("銘柄別財務情報保存完了", "code", code, "count", count)
		}
	}
...
// UpdateStatementsByDate 指定された日付の財務情報を取得し、DBに保存
func (s *StatementsService) UpdateStatementsByDate(date string, debug bool) error {
	if date == "" {
	...
	statements, err := s.client.StatementsClient.GetAllStatements(idToken, "", date, debug)
	if err != nil {
		return fmt.Errorf("財務情報取得エラー: %v", err)
	}
...
func (s *StatementsService) UpdateStatementsByCodeAndDate(code, date string, debug bool) error {
	...
	if len(statements.Statements) > 0 {
		if count, err := s.repository.SaveFinancialStatements(statements); err != nil {
			...
			slog.Info("銘柄・日付別財務情報保存完了", "code", code, "date", date, "count", count)
		}
	}
...
func (s *StatementsService) UpdateStatementsMultipleDates(codes []string, dates []string, intervalMs int, debug bool) error {
	...
	for i, date := range dates {
		...
		if len(codes) == 0 {
			// 全銘柄取得
			err := s.UpdateStatementsByDate(date, debug)
			if err != nil {
			...
			for _, code := range codes {
				err := s.UpdateStatementsByCodeAndDate(code, date, debug)
				if err != nil {
