...
func (c *StatementsClient) GetStatements(idToken, code, date string, debug bool) (*schema.FinancialStatementsResponse, error) {
	return c.GetStatementsWithPagination(idToken, code, date, "", debug)
}

// GetStatementsWithPagination pagination_keyを指定して財務情報を取得
func (c *StatementsClient) GetStatementsWithPagination(idToken, code, date, paginationKey string, debug bool) (*schema.FinancialStatementsResponse, error) {
	// パラメータの構築
	params := url.Values{}
	if code != "" {
	...
	// レスポンスのデコード
	var financialStatementsResp schema.FinancialStatementsResponse
...
func (c *StatementsClient) GetAllStatements(idToken, code, date string, debug bool) (*schema.FinancialStatementsResponse, error) {
	var allStatements []schema.FinancialStatement
	paginationKey := ""
	...
	return &schema.FinancialStatementsResponse{
