...
.PHONY: all build build-sa build-migrate build-jquants clean clean-sa clean-migrate clean-jquants test deps update-deps tidy help cross-build info
...
help:
	...
	@echo "  all          - clean + deps + build (sa + migrate + jquants)"
	@echo "  build        - 全バイナリをビルド (sa + migrate + jquants)"
	@echo "  build-sa     - saコマンドをビルド"
	@echo "  build-migrate - migrateコマンドをビルド"
	@echo "  build-jquants - jquantsコマンドをビルド"
	@echo "  clean        - 全ビルドファイルを削除"
	@echo "  clean-sa     - saコマンドのビルドファイルを削除"
	@echo "  clean-migrate - migrateコマンドのビルドファイルを削除"
	@echo "  clean-jquants - jquantsコマンドのビルドファイルを削除"
...
# 全バイナリビルド
build: build-sa build-migrate build-jquants
...
# jquantsコマンドビルド
build-jquants: $(BUILD_DIR)
	@echo "jquantsコマンドをビルド中..."
	cd jquants && $(GOBUILD) $(BUILD_FLAGS) -o ../$(BUILD_DIR)/jquants .
	@echo "jquantsコマンドビルド完了: $(BUILD_DIR)/jquants"

# 全クリーンアップ
clean: clean-sa clean-migrate clean-jquants
...
# jquantsコマンドクリーンアップ
clean-jquants:
	@echo "jquantsコマンドをクリーンアップ中..."
	cd jquants && $(GOCLEAN)
	@echo "jquantsコマンドクリーンアップ完了"
...
cross-build: $(BUILD_DIR)
	...
	@for platform in $(PLATFORMS); do \
		...
		JQUANTS_OUTPUT=$(BUILD_DIR)/jquants-$$OS-$$ARCH; \
		if [ $$OS = "windows" ]; then \
			SA_OUTPUT=$$SA_OUTPUT.exe; \
			MIGRATE_OUTPUT=$$MIGRATE_OUTPUT.exe; \
			JQUANTS_OUTPUT=$$JQUANTS_OUTPUT.exe; \
		fi; \
		echo "ビルド中 ($$OS/$$ARCH): $$SA_OUTPUT, $$MIGRATE_OUTPUT, $$JQUANTS_OUTPUT"; \
		...
		cd ../jquants && GOOS=$$OS GOARCH=$$ARCH $(GOBUILD) $(BUILD_FLAGS) -o ../$$JQUANTS_OUTPUT .; \
		cd ..; \
	done
...
info:
	...
	@echo "  jquants/   - J-Quants API連携CLIツール"
